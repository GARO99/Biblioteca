...
@router.post("/", response_model=LoanRead, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_employee)])
...
@router.post("/{loan_id}/return", response_model=LoanRead, dependencies=[Depends(require_employee)])
...
@router.get("/", response_model=List[LoanRead], dependencies=[Depends(require_employee)])
...
@router.get("/{loan_id}", response_model=LoanRead, dependencies=[Depends(require_employee)])
def get_loan(loan_id: UUID, uow: UnitOfWork = Depends(get_uow)):
    svc = LoanService(uow)
    return svc.get(loan_id)

@router.put("/{loan_id}", response_model=LoanRead, dependencies=[Depends(require_employee)])
...
@router.delete("/{loan_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_employee)])
