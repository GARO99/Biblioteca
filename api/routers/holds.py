...
@router.post("/", response_model=HoldRead, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_employee)])
...
@router.post("/{hold_id}/cancel", response_model=HoldRead, dependencies=[Depends(require_employee)])
def cancel_hold(hold_id: UUID, uow: UnitOfWork = Depends(get_uow)):
    svc = HoldService(uow)
    return svc.cancel(hold_id)

@router.post("/{hold_id}/fulfill", response_model=HoldRead, dependencies=[Depends(require_employee)])
...
@router.get("/", response_model=List[HoldRead], dependencies=[Depends(require_employee)])
...
@router.get("/{hold_id}", response_model=HoldRead, dependencies=[Depends(require_employee)])
def get_hold(hold_id: UUID, uow: UnitOfWork = Depends(get_uow)):
    svc = HoldService(uow)
    return svc.get(hold_id)

@router.delete("/{hold_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_employee)])
